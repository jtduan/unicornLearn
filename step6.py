...
def hook_memory_write(uc, address, size, value, user_data, extra):
    print("write: 0x%x" % (size))

def hook_memory_read(uc, address, size, value, user_data, extra):
    print("read: 0x%x" % (size))
...
# Todo:还存在bug
def initializeTLS(mu):
    thread = allocateStack(0x400)
    __stack_chk_guard = allocateStack(4)
    programName = allocateStack(0x100)
    programNamePointer = allocateStack(4)
    mu.mem_write(programNamePointer, programName.to_bytes(4, byteorder='little'))

    auxv = allocateStack(0x100)
    mu.mem_write(auxv, int(25).to_bytes(4, byteorder='little'))
    mu.mem_write(auxv + 4, __stack_chk_guard.to_bytes(4, byteorder='little'))

    environ = allocateStack(8)
    mu.mem_write(environ, int(0).to_bytes(4, byteorder='little'))

    argv = allocateStack(0x100)
    mu.mem_write(argv + 4, programNamePointer.to_bytes(4, byteorder='little'))
    mu.mem_write(argv + 8, environ.to_bytes(4, byteorder='little'))
    mu.mem_write(argv + 12, auxv.to_bytes(4, byteorder='little'))

    tls = allocateStack(0x80 * 4)
    mu.mem_write(tls + 4, thread.to_bytes(4, byteorder='little'))
    mu.mem_write(tls + 12, argv.to_bytes(4, byteorder='little'))
    mu.reg_write(UC_ARM_REG_C13_C0_3, tls)
...
try:
    ...
    mu.hook_add(UC_HOOK_MEM_WRITE, hook_memory_write)
    mu.hook_add(UC_HOOK_MEM_READ, hook_memory_read)
